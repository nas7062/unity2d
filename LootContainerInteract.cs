...
public class LootContainerInteract : Interactable,IPersistant
{
	...
	public void Open(Character character)
	{
		...
		character.GetComponent<ItemContainerInterectController>().Open(itemContainer, transform);
	}
	...
	public void Close(Character character)
	{
		...
		character.GetComponent<ItemContainerInterectController>().Close();
	}
	...
	public class SaveLootItemData
	{
		...
		public SaveLootItemData(int id,int cnt)
		{
			itemId = id;
			count = cnt;
		}
	...
	public void Load(string jsonString)
	{
		...
		for(int i =0; i< toLoad.itemDatas.Count; i++)
		{
			...
			else
			{
				...
				GameManager.instance.itemDB.items[toLoad.itemDatas[i].itemId];
				itemContainer.slots[i].count =
				toLoad.itemDatas[i].count;
			}
		}
	}
}
